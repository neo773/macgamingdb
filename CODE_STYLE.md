...
- Code is self-documenting: names carry the meaning a comment would. Descriptive, intent-revealing names for variables, functions, methods, types — and function parameters, including lambda/callback parameters. `reviews.filter((review) => ...)`, never `reviews.filter((r) => ...)`; `(event)`, never `(e)`; `(error)`, never `(err)`. No abbreviations, ever, at any length. Booleans read `isX`/`hasX`. One term per concept codebase-wide. Pluralize collections, singular for one entity. `generate` for creating values; never `get` for something that mutates. No `v2` suffixes or leaked internal qualifiers.
