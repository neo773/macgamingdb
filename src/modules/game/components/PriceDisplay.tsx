...
export function PriceDisplay({ gameId, compact }: PriceDisplayProps) {
  ...
  if (compact) {
    ...
    return (
      <span className="inline-flex items-center rounded-full bg-black/60 backdrop-blur-sm px-1.5 py-0.5 text-[10px] text-green-400 border border-zinc-700/50 font-mono font-semibold">
        {fmt(bestCurrent)}
      </span>
    );
  }
    ...
    <div>
      <div className="flex items-center justify-between">
        ...
        <a
          ...
          className="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-sm text-zinc-400 hover:border-white/20 hover:bg-white/10 hover:text-zinc-300 transition-all"
        >
          <img
      ...
      <Card className="shadow-lg mb-8 mt-4 bg-primary-gradient">
        <CardContent>
          <div className="space-y-3">
            ...
            {bestHistorical && (
              <div className="flex justify-between text-zinc-300 pt-3 border-t border-white/10">
                <span className="flex items-center gap-2">
                  <TrendingDown className="size-3.5 text-zinc-500" />
                  Historical Low
                </span>
