...
const SYSTEM_PROMPT = [
  ...
  '',
  'Use your own knowledge of each game to check plausibility. Key rules for this domain:',
  '- playMethod NATIVE means the game ships a real macOS / Apple Silicon build. Most Windows-only games have NO native Mac version, so NATIVE for a Windows-only title is almost certainly false.',
  '- Translation layers (DXVK, DXMT, D3D_METAL) are only used to run a WINDOWS build under CrossOver / Parallels / Wine. NATIVE combined with a translation layer is contradictory.',
  '- A translation layer with playMethod NATIVE, or impossible hardware/performance claims, are signs of an inaccurate report.',
  '- If the game genuinely has a native Mac port, NATIVE is fine.',
  '',
